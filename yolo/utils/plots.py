...
#         fs / 10), font_size=fs, pil=True, example=names)
#     for i in range(i + 1):
#         x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
#         annotator.rectangle([x, y, x + w, y + h], None,
#                             (255, 255, 255), width=2)  # borders
#         if paths:
#             annotator.text([x + 5, y + 5], text=Path(paths[i]
#                                                      ).name[:40], txt_color=(220, 220, 220))  # filenames
#         if len(targets) > 0:
...
#     # plt.subplots_adjust(wspace=0.05, hspace=0.05)
#     for i in range(n):
#         ax[i].imshow(blocks[i].squeeze().permute(
#             (1, 2, 0)).numpy().clip(0.0, 1.0))
#         ax[i].axis('off')
...
#                         ' '.join(f'{names[i]:3s}' for i in pred[:nmax]))
#     return f
