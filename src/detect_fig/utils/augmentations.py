...
class Albumentations:
    ...
    def __init__(self, size=640):
        ...
        try:
            ...
            T = [
                A.RandomResizedCrop(height=size, width=size, scale=(
                    0.8, 1.0), ratio=(0.9, 1.11), p=0.0),
                A.Blur(p=0.01),
                A.MedianBlur(p=0.01),
                A.ToGray(p=0.01),
                A.CLAHE(p=0.01),
    ...
    try:
        ...
        if augment:  # Resize and crop
            ...
            if auto_aug:
                ...
                if jitter > 0:
                    # repeat value for brightness, contrast, satuaration, 0 hue
                    color_jitter = (float(jitter), ) * 3
                    T += [A.ColorJitter(*color_jitter, 0)]
        else:  # Use fixed crop for eval set (reproducibility)
            T = [A.SmallestMaxSize(max_size=size),
...
def classify_transforms(size=224):
    ...
    assert isinstance(
        size, int), f'ERROR: classify_transforms size {size} must be integer, not (list, tuple)'
...
class LetterBox:
    ...
    def __call__(self, im):  # im = np.array HWC
        ...
        hs, ws = (math.ceil(x / self.stride) *
                  self.stride for x in (h, w)) if self.auto else self.h, self.w
        top, left = round((hs - h) / 2 - 0.1), round((ws - w) / 2 - 0.1)
        im_out = np.full((self.h, self.w, 3), 114, dtype=im.dtype)
        im_out[top:top + h, left:left +
               w] = cv2.resize(im, (w, h), interpolation=cv2.INTER_LINEAR)
        return im_out
